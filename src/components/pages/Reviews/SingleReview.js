...
const SingleReview = ({ review }) => {
    return (
        <div key={review._id} className="flex flex-col max-w-sm mx-4 my-6 shadow-lg">
            ...
            <div className="flex flex-col items-center justify-center p-8 rounded-b-lg dark:bg-violet-400 dark:text-gray-900">
                <img src={review?.userImage} alt="" className="w-16 h-16 mb-2 -mt-16 bg-center bg-cover rounded-full dark:bg-gray-700" />
                <p className="text-xl font-semibold leading-tight">{review?.userName}</p>
                <p>Reviewd On: <span className='font-bold'>{review?.time}</span></p>
